...
# result = contract.wage * 0.10""",  # noqa: E501
    ...
    def _compute_rule(self, localdict):
        ...
        """
        self.ensure_one()
        # [20.0] This used to dispatch through `odoo.api.call_kw`, which is the
        # RPC entry point, not a dispatcher: it has moved to
        # `odoo.service.model` and now refuses private method names outright.
        # Looking the method up on the recordset does the same thing directly.
        method = f"_compute_rule_{self.amount_select}"
        return getattr(self, method)(localdict)
    ...
    def _compute_rule_code(self, localdict):
        try:
            # [20.0] `safe_eval` lost `nocopy`: the context it is given is always
            # mutated now, which is exactly what `nocopy=True` asked for. The rule
            # still reads its result back out of `localdict`.
...
"""
    ...
    def _satisfy_condition_python(self, localdict):
        try:
            # [20.0] See `_compute_rule_code`: `nocopy` is gone and implied.
