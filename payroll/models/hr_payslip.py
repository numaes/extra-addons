...
class HrPayslip(models.Model):
    ...
    def _compute_leave_days(self, contract, day_from, day_to):
        ...
        # [20.0] `resource.calendar` no longer carries a timezone; the contract
        # does. `hr.version.tz` is required, so there is always one.
        tz = timezone(contract.tz or "UTC")
        day_leave_intervals = contract.employee_id.list_leaves(
            day_from, day_to, calendar=contract.resource_calendar_id
        )
        for day, hours, leave in day_leave_intervals:
            holiday = leave[:1].holiday_id
            # [20.0] `hr.leave.type` was folded into `hr.work.entry.type`: a
            # leave now names its type directly through `work_entry_type_id`,
            # where it used to reach a work entry type through
            # `holiday_status_id`. A leave with no type is still a global one.
            leave_type = holiday.work_entry_type_id
            current_leave_struct = leaves.setdefault(
                leave_type,
                {
                    "name": leave_type.name or _("Global Leaves"),
                    "sequence": leave_type.sequence if leave_type else 5,
                    "code": leave_type.code if leave_type else "GLOBAL",
                    "number_of_days": 0.0,
                    "number_of_hours": 0.0,
    ...
    def get_payroll_dict(self, contracts):
        ...
        :param contracts: Recordset of all hr.version records in this payslip
    ...
    def get_payslip_vals(
        ...
        if not self.env.context.get("contract"):
            contract_ids = employee.version_id.ids
        else:
            if contract_id:
                contract_ids = [contract_id]
            else:
                contract_ids = employee._get_employee_contract_versions(
                    date_from, date_to
                ).ids
        if not contract_ids:
            return res
    ...
    def _get_employee_contracts(self):
        ...
        for payslip in self:
            if payslip.contract_id.ids:
                ...
                contracts |= payslip.employee_id._get_employee_contract_versions(
