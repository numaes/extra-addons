...
class TestPayslipBase(TransactionCase):
    def setUp(self):
        ...
        self.richard_emp = self.env["hr.employee"].create(
            {
                "name": "Richard",
                # [20.0] `hr.employee.gender` was dropped from the core.
                "birthday": "1984-05-01",
        ...
        self.richard_contract = self._set_contract(
            self.richard_emp,
            {
        ...
        self._set_contract(
            self.sally,
            {
                "contract_date_start": Date.today().strftime("%Y-%m-1"),
    ...
    def _set_contract(self, employee, values):
        """[20.0] Give an employee the contract terms in `values`.

        `hr.contract` became `hr.version`, and a contract is no longer a record
        of its own: creating an employee already creates a version, and versions
        are unique per employee and `date_version`. So setting up a contract
        means writing the terms onto the version the employee already has.
        A *second* contract is a second version, which `create_version` builds
        from a later `date_version` — pass one in `values` to get that.
        """
        values = dict(values, employee_id=employee.id)
        if values.get("date_version"):
            return employee.create_version(values)
        version = employee.version_id
        version.write(values)
        return version

    def apply_contract_cron(self):
        """[20.0] A no-op, kept so the tests still read as they did.

        `hr_contract` shipped a cron that walked contracts from draft to open to
        expired by their dates. `hr.contract` no longer exists: a contract is an
        `hr.version`, which has no state at all — it is simply effective from its
        `date_version`. There is nothing left for a cron to advance.
        """
