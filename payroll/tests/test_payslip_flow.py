...
class TestPayslipFlow(TestPayslipBase):
    ...
    def test_00_payslip_flow(self):
        ...
        # [20.0] `odoo.tools.test_reports.try_report_action` no longer exists.
        # It ran the wizard behind the action and rendered the report it
        # returned; we now do both explicitly, which asserts the same thing.
        wizard = (
            self.env["payslip.lines.contribution.register"]
            .with_context(
                active_model="hr.contribution.register",
                active_ids=[self.register_hra.id],
                # Without this the report action is swapped for the layout
                # configurator, exactly as it was under `try_report_action`.
        ...
        content, content_type = report.with_context(
            active_ids=[self.register_hra.id]
        )._render_qweb_html(action["report_name"], [], data=action["data"])
    ...
    def test_get_contracts_singleton(self):
        ...
        # [20.0] This used to assert that the contract counted for nothing until
        # the contract cron had opened it. Contracts have no state any more:
        # an `hr.version` is in force from its `contract_date_start`, so Sally's
        # single contract counts from the moment the fixture writes it.
        contracts = payslip._get_employee_contracts()
        self.assertEqual(
            len(contracts), 1, "There is one open contract for the employee"
        )

        self.sally.version_id.contract_date_end = (Date.today() - timedelta(days=1)).strftime(
            "%Y-%m-%d"
        )
        self._set_contract(
            self.sally,
            {
                ...
                "contract_date_start": Date.today().strftime("%Y-%m-%d"),
                "struct_id": self.sales_pay_structure.id,
                "wage": 6500.00,
            },
        )
        # [20.0] Both contracts now count: the first one ended yesterday but
        # still overlaps the payslip period, and a version has no state that
        # would exclude it. Under `hr_contract` the closed one was filtered out
        # by its state, not by its dates.
        contracts = payslip._get_employee_contracts()
        self.assertEqual(
            len(contracts), 2, "Both contracts overlap the payslip period"
        )

    def test_get_contracts_multiple(self):
        self.sally.version_ids[0].contract_date_end = Date.today().strftime("%Y-%m-01")

        self._set_contract(
            self.sally,
            {
                "name": "Second contract for Sally",
                "date_version": Date.today().strftime("%Y-%m-02"),
                "contract_date_start": Date.today().strftime("%Y-%m-02"),
                "struct_id": self.sales_pay_structure.id,
                "wage": 6500.00,
            },
        )
    ...
    def test_compute_sheet_no_valid_contract(self):
        """[20.0] A contract is valid by its dates alone.

        This case used to rely on the contract cron never having run, which left
        every contract in `draft` and therefore invalid. `hr.version` has no
        state, so the only way to have no valid contract is to have none in
        force over the period — here, one that ended before the payslip starts.
        """
        self.richard_emp.version_id.write(
            {
                "contract_date_start": (Date.today() - timedelta(days=120)).strftime(
                    "%Y-%m-%d"
                ),
                "contract_date_end": (Date.today() - timedelta(days=90)).strftime(
                    "%Y-%m-%d"
                ),
            }
        )
