...
class TestAccountStatementImportFile(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.bank_account = cls.env["res.partner.bank"].create(
            {"account_number": "1111111111", "partner_id": cls.env.company.partner_id.id}
        )
        cls.bank_account_2 = cls.env["res.partner.bank"].create(
            {"account_number": "3333333333", "partner_id": cls.env.company.partner_id.id}
        )
        ...
        # [20.0] A Binary field no longer accepts raw `bytes`: it takes a
        # `BinaryValue` or the base64 STRING that RPC sends. `b64encode` returns
        # bytes, hence "use BinaryValue instead of bytes".
        file = base64.b64encode(open(f_path, "rb").read()).decode()
        cls.import_wizard = (
