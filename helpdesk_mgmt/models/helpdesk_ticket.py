...
class HelpdeskTicket(models.Model):
    ...
    # [20.0] `mail.thread.cc` was removed from Odoo 20 along with the `email_cc`
    # field it added; the core did the same to `crm.lead` and `hr.applicant`, which
    # were its other users. What is lost is the CC handling on INCOMING mail, not
    # the chatter: `mail.tracking.duration.mixin` below already inherits
    # `mail.thread`, so naming it here as well is what C3 refuses. The `email_cc`
    # fields in `data/helpdesk_data.xml` are `mail.template`'s own and are
    # unaffected.
    _inherit = [
        "mail.activity.mixin",
        "portal.mixin",
    ...
    def _notify_get_reply_to(self, default=None, author_id=False):
        ...
        [20.0] The core signature gained `author_id`; an override that does not
        accept it raises TypeError on every notified message, which stopped even
        the module's own demo data from loading.
        """
