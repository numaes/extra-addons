...
class CustomerPortalHelpdesk(CustomerPortal):
    ...
    def portal_my_tickets(
        ...
        **kw,
        ...
        # [20.0] `ir.rule` no longer exists — permissions and record rules were
        # unified in `ir.access` — and the domain a user is allowed to read is now
        # `Model._access_domain(operation)`.
        domain = Domain.AND([domain, HelpdeskTicket._access_domain("read")])
